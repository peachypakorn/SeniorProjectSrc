function [phsCmpO] = lightfieldStitchV2(ifiles, oprefix, runParam)
    ht = runParam.ht;
    nA = length(ifiles);
    ...
    if expShear,
        ...
        for curLev = keyLev:-1:runParam.shearMinLev,
            ...
            %if false
                phsCmp = zeros([gC, gR]);
                for iR = 1:gR,
                    cR = round((iR - 0.5) * nR / gR + 0.5);
                    for iC = 1:gC,
                        cC = round((iC - 0.5) * nL / gC + 0.5);
                        rang = max(cC-hs, 1):min(cC+hs, nL);
                        val = sum(sum(pyrCmp1(inds(rang), cR, :) .* conj(pyrCmp2(inds(rang), cR, :))));
                        phsCmp(iC, iR) = angle(val) / (corrDist * po2);
                    end
                end
            %end
            %phsCmp = angle(sum(imresize(pyrCmp1(inds, :, :) .* conj(pyrCmp2(inds, :, :)), [gC, gR]), 3)) / (corrDist * po2);

            %maxPhs = runParam.maxShear / (corrDist*depPhsRatios{keyLev});
            if ~isscalar(phsCmpA),
                phsCmpA = imresize(phsCmpA, [gC, gR]);
                phsCmpAO = phsCmpA;
                phsCmpA = phsCmp + (2*pi / (corrDist*po2)) * round((phsCmpA - phsCmp) / (2*pi / (corrDist*po2)));
                %phsCmpA(phsCmpA > maxPhs) = phsCmpAO(phsCmpA > maxPhs);
                %phsCmpA(phsCmpA < -maxPhs) = phsCmpAO(phsCmpA < -maxPhs);
            else
                phsCmpA = phsCmp;
                %phsCmpA(phsCmpA > maxPhs) = maxPhs;
                %phsCmpA(phsCmpA < -maxPhs) = -maxPhs;
            end
        end
        phsCmpA = phsCmpA;
