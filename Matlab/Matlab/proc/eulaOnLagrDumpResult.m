function [synth, mem, phsCmpO, dump] = eulaOnLagrDumpResult(img1, img2, depthLo, depthRo, testParam, runParam, mem, iA, noEula, phsCmpO)
    rho = runParam.rho;
    ht = min(runParam.ht, ceil(log(size(img1, 2)) / log(2) + 1e-6) - 4);
    nA = runParam.nA;
    dA = runParam.dA;
    vis = struct();
    dump = struct();
    ...
    dump.img1 = img1;
    dump.img2 = img2;
    dump.depthLo = depthLo;
    dump.depthRo = depthRo;
    dump.nA = nA;
    dump.dA = dA;
    dump.iA = iA;
    dump.rho = rho;
    ...
    if ~isstruct(mem),
        ...
        dump.pyr1R = pyr1R;
        dump.pyr2R = pyr2R;
        dump.preDepth1 = preDepth1;
        dump.preDepth2 = preDepth2;
        dump.refInd1 = refInd1;
        dump.refInd2 = refInd2;
        ...
        dump.preDepth1 = preDepth1;
        dump.preDepth2 = preDepth2;
        dump.phaseDepth1 = phsDif1;
        dump.phaseDepth2 = phsDif2;
        for k = nLev-1:-1:1,
            ind = pyrBandIndices(pind,k);
            dump.phaseDepth1(ind, :) = dump.phaseDepth1(ind, :) * depPhsRatios{k};
            dump.phaseDepth2(ind, :) = dump.phaseDepth2(ind, :) * depPhsRatios{k};
        end        
        ...
        dump.postDepth1 = depth1;
        dump.postDepth2 = depth2;
    ...
    for c = 1:nCol,
        ...
        if ~zipzapBlend,
            [pyrRec, bigPyr] = synthWrap(mem, c, nA, iA, dA, rho, recR, pind, filters, 1, twoSideInterp, depPhsRatios);
            if expBlend,
                if iA == 1 || iA == 2*nA,
...
function [pyrRec, bigPyr] = synthWrap(mem, c, nA, iA, dA, rho, recR, pind, filters, minLev, twoSideInterp, depPhsRatios)
    amp = (nA+0.5-iA)*dA;
    ...
    if (amp > 0.5 && twoSideInterp) || (amp > 0 && ~twoSideInterp),
        ...
        [pyrRec, weight1, bigPyr] = nufftWarp(pyr1(:, :) .* AA1, ...
...
function indices =  pyrBandIndicesExt(pind,band)
    indices = (pind(band, 1)*2-1):(pind(band, 1)*2-1 + pind(band, 2)*2 - 1);
end
...
function [resPyr, weight, bigPyr] = nufftWarp(pyr, mov, depth, pind, filters, m, Q, depPhsRatios, direction, minLev)
    %http://www.cscamm.umd.edu/programs/fam04/qing_liu_fam04.pdf
    nLev = max(size(filters));
    nR = size(pyr, 2);
    n = length(filters{1});
    resPyr = zeros(size(pyr));
    bigPyr = zeros(size(pyr,1)*2, size(pyr,2));
    ...
    for rSt = 1:50:nR,
        ...
        for k = nLev:-1:minLev,
            ...
            indicesExt = pyrBandIndicesExt(pind, k);
            %if k == nLev,
            %    mov(indices, :) = 0;
            %end
            if false%k == nLev,
            ...
            bigPyr( indicesExt, rInd) = sig;
            for iR = 1:rSize,                
                if (k==1 && (rSt+iR-1) == 60),
                    f1.displacement = mov(indices,60);
                    f1.pyrlevel     = pyr(indices, 60);
                    f1.nufftBefore  = sig(:, iR);
                    save('nufft_only.mat', 'f1');
                end
