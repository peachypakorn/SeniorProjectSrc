...
int main()
{
	static hls::stream< t_nufft_output_complex > xn_input("in");
		static hls::stream< t_nufft_output_complex> xk_output("out");
...
//		int ndat = 256 ;
		for(int j=0;j<4;j++)
		{
			...
			for(int i=0;i<ndat/2;i++) {
				...
				fprintf(fo, " %.8f %.8f \n ", valOut1.real().to_double(), valOut1.imag().to_double());
