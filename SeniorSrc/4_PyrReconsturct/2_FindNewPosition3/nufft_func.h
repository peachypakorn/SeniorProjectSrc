...
void nufft_top(hls::stream<t_input_complex>  sig[C],
...
//#define DEBUG
	...
	t_nufft_output_complex sigMod0[C][MAXWIDTH * M /4];
	t_nufft_output_complex sigMod1[C][MAXWIDTH * M /4];
	t_nufft_output_complex sigMod2[C][MAXWIDTH * M /4];
	t_nufft_output_complex sigMod3[C][MAXWIDTH * M /4];
...
#pragma HLS unroll
			...
			t_nufft_output_complex pyrVal = sig[c].read();
...
#pragma HLS data_pack variable=pyrValX
			...
			sigMod0[c][ nIdx0 ] += pyrValX[0];
			sigMod1[c][ nIdx1 ] += pyrValX[1];
			sigMod2[c][ nIdx2 ] += pyrValX[2];
			sigMod3[c][ nIdx3 ] += pyrValX[3];
		}
	}
	for(int i=0;i<nL * M/4;i++ ) {
...
#pragma HLS PIPELINE II=4
		for(int c=0;c<C;c++) {
#pragma HLS unroll
			sigStreamOut[c].write( sigMod0[c][i]);
			sigStreamOut[c].write( sigMod1[c][i]);
			sigStreamOut[c].write( sigMod2[c][i]);
			sigStreamOut[c].write( sigMod3[c][i]);
		}
	}
}
