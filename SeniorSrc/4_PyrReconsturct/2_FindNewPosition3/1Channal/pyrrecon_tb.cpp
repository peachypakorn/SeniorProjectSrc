...
//	printf("%.8f %.8f\n", c0[1][361].real().to_float(), c0[1][361].imag().to_float());
	FILE *f2 = fopen("inputSignal.txt", "wb");
	for(int round =0;round<nRound;round++){
		for(int i=0;i<1520;i++) {
			t_input_complex v;
				if (i<1504){
...
//						   sigStreamFFTOutH, sigStreamFFTOutL0, sigStreamFFTOutLA);
	...
	for(int round =0;round<nRound;round++) {
			...
			for(int i=0;i<1024;i++) {
				t_nufft_output_complex v = sigStreamOutH.read();
				if (1) {
	//				std::cout << v << std::endl;
					fprintf(fo, "%.8f %.8f\n", v.real().to_float(), v.imag().to_float());
				}
			...
			for(int i=0;i<1024;i++) {
				t_nufft_output_complex v = 	sigStreamOutL0.read();
				if (1) {
					//std::cout << v << std::endl;
					fprintf(fo, "%.8f %.8f\n", v.real().to_float(), v.imag().to_float());
				}
			...
			for(int i=0;i<960;i++) {
				t_nufft_output_complex v = 	sigStreamOutLA.read();
				if (1) {
					//std::cout << v << std::endl;
					fprintf(fo, "%.8f %.8f\n", v.real().to_float(), v.imag().to_float());
				}
			...
			for(int i=0;i<16;i++) {
				t_nufft_output_complex v = 	sigStreamOutLP.read();
				if (1) {
					//std::cout << v << std::endl;
					fprintf(fo, "%.8f %.8f\n", v.real().to_float(), v.imag().to_float());
				}
