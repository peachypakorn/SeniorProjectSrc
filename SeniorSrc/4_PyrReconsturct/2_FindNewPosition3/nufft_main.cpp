...
#pragma HLS DATAFLOW
	hls::stream<t_input_complex>  sigH[C];
	hls::stream<t_input_complex>  sigL0[C];
	hls::stream<t_input_complex>  sigLA[C];
...
//#pragma HLS stream variable=sigStreamOutLP   depth=64
	...
	hls::stream<t_input_complex>  sigInMem[C];
...
#pragma HLS pipeline
		for(int c=0;c<C;c++) {
			t_input_complex v = sigInMem[c].read();
			if (coefIdx >=0 && coefIdx < climits[1]) 			sigH[c].write(v);
			if (coefIdx >=climits[1] && coefIdx < climits[2])	sigL0[c].write(v);
			if (coefIdx >=climits[2] && coefIdx < climits[6])   sigLA[c].write(v);
			if (coefIdx >=climits[6])                           sigStreamOutLP[c].write(v);
