...
template < int NUMIN>
void  S1_S2Multiplex( 	hls::stream<t_nufft_output_complex > S1Out_H[NUMIN],
						hls::stream<t_nufft_output_complex > S1Out_L0[NUMIN],
						hls::stream<t_nufft_output_complex > S1Out_LA[NUMIN],
						hls::stream<t_nufft_output_complex > S1Out_LP[NUMIN],
...
#pragma HLS PIPELINE
			...
			S2In_H.write( S1Out_H[inputC].read());
...
//#pragma HLS DATAFLOW
		for(int i=0;i<1024;i++) {
#pragma HLS PIPELINE
			S2In_L0.write( S1Out_L0[inputC].read());
...
#pragma HLS PIPELINE
			S2In_LA.write( S1Out_LA[inputC].read());
...
//#pragma HLS DATAFLOW
		for(int i=0;i<16;i++) {
#pragma HLS PIPELINE
			S2In_LP.write( S1Out_LP[inputC].read());
